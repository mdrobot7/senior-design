...
module core_m_unit_test;
  ...
  reg[`WORD_WIDTH-1:0] stream_slave_read;
  ...
  stream_slave_m #(.SIZE(`MAILBOX_STREAM_SIZE), .BUFFER_SIZE(2*`CORE_OUTBOX_HEIGHT)) outbox_slave (
    .clk_i(clk),
  ...
  `SVTEST(test_lui_lli)
    ...
    `FAIL_UNLESS_EQUAL(outbox_slave.buffer[8], 32'h0);
    `FAIL_UNLESS_EQUAL(outbox_slave.buffer[9], 32'h0);
    `FAIL_UNLESS_EQUAL(outbox_slave.buffer[10], 32'h0);
    `FAIL_UNLESS_EQUAL(outbox_slave.buffer[11], 32'h0);
    `FAIL_UNLESS_EQUAL(outbox_slave.buffer[12], 32'h0);
  ...
  `SVTEST_END
    ...
    for (i = 0; i < 100; i++) begin
      clk_rst.WAIT_CYCLES(1);
      if (outbox_slave.buffer_size == 2*`CORE_OUTBOX_HEIGHT)
        break;
    end
    `FAIL_UNLESS_EQUAL(outbox_slave.buffer_size, 2*`CORE_OUTBOX_HEIGHT);
  ...
  `SVTEST(mem_test)
    ...
    for(i = 0; i < 50; i = i + 1) begin
      @(negedge clk);
      if(stallo && !my_core_m.halt_stall) begin
        @(negedge stallo);
        @(negedge clk);
      end
      inst = i_mem[i];
  ...
  `SVTEST(outbox_test)
    integer i;
    $readmemh("mem_files/outbox_test.mem", i_mem);

    inst = 0;
    while(outbox_slave.buffer_size > 0) begin
      outbox_slave.READ(stream_slave_read);
    end
    clk_rst.RESET();
    clk_rst.WAIT_CYCLES(3);
    ...
    for(i = 0; i < `CORE_OUTBOX_HEIGHT; i = i + 1) begin
      outbox_slave.READ(stream_slave_read);
      `FAIL_UNLESS_EQUAL(stream_slave_read, i + 1);
    end
  `SVTEST_END
