...
module regfile_m_unit_test;
  ...
  localparam CORE_HEIGHT = 48;
  localparam BASE_ADDR = 16;
  localparam NO_ZERO_REG = 1;

  wire clk, nrst;
  clk_rst_m clk_rst(.clk_o(clk), .nrst_o(nrst));
  reg wr_en;
  reg[REG_WIDTH-1:0] wr_data;
  reg[5:0] wr_addr, r1_addr, r2_addr;

  wire[`WORD_WIDTH-1:0] r1_data, r2_data;

  reg [REG_WIDTH-1:0] test_regfile [CORE_HEIGHT+BASE_ADDR-1:BASE_ADDR];
  ...
  `SVTEST(main)
    ...
    for(i = 0; i < 100; i = i + 1) begin
      ...
      wr_addr <= {$random};
      @(posedge clk);
      if((wr_en == 1) && (wr_addr != CORE_HEIGHT + BASE_ADDR -1)) begin
