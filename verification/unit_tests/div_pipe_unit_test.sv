...
module div_pipe_m_unit_test;
  ...
  reg  [`STREAM_SIPORT(5 + SIZE * 2)] sstreamai;
  wire [`STREAM_SOPORT(5 + SIZE * 2)] sstreamao;

  reg  [`STREAM_MIPORT(5 + SIZE)] mstreamai;
  wire [`STREAM_MOPORT(5 + SIZE)] mstreamao;
  ...
  stream_master_m #(5 + SIZE * 2) pipe0_master(
    .clk_i(clk),
  ...
  stream_slave_m #(5 + SIZE, 1) pipe0_slave(
    .clk_i(clk),
  ...
  begin
      ...
      pipe0_master.WRITE({ extra, a, b });
      pipe0_slave.READ({ extrao, y });

      `FAIL_UNLESS_EQUAL(extra, extrao);
  end
  endtask
  ...
  begin
      ...
      pipe0_master.WRITE_LAST({ extra, a, b });
      pipe0_slave.READ({ extrao, y });
