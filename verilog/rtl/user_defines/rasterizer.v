...
`define RAST_BARY_OUT_WITH (`SC_WIDTH * 2 + `WORD_WIDTH * 3)
`define RAST_INTERP_OUT_WIDTH (`SC_WIDTH * 2 + `WORD_WIDTH * 4)
`define RAST_DT_OUT_WIDTH (`SC_WIDTH * 2 + `WORD_WIDTH * 3)
`define RAST_WAVG_OUT_WIDTH (`SC_WIDTH * 2 + `WORD_WIDTH * 8)

`define LERPINATOR_IN_PACKET(l0, l1, l2, v0, v1, v2) { (l0), (l1), (l2), (v0), (v1), (v2) }

`define LERPINATOR_IN_WIDTH (`WORD_WIDTH * 3 + `WORD_WIDTH * 3)
`define LERPINATOR_IN (`LERPINATOR_IN_WIDTH - 1):0

`define LERPINATOR_OUT_WIDTH (`WORD_WIDTH)
`define LERPINATOR_OUT (`LERPINATOR_OUT_WIDTH - 1):0

`define VERTEX_ORDER_TARGET_CACHE `NUM_CORES
`define VERTEX_ORDER_WIDTH ($clog2(`NUM_CORES + 1))

`define SHADED_VERTEX_WIDTH (32 * 12)
