...
`define CORE_OUTBOX_HEIGHT  (12)
`define OUTBOX_STREAM_CYCLES ((`WORD_WIDTH*`CORE_OUTBOX_HEIGHT) /  `MAILBOX_STREAM_SIZE)
`define OUTBOX_COUNTER_WIDTH ($clog2(`OUTBOX_STREAM_CYCLES + 1))

`define CORE_INBOX_HEIGHT   (8)
`define CORE_MAILBOX_HEIGHT (8)
`define MAILBOX_STREAM_CYCLES ((`WORD_WIDTH*`CORE_MAILBOX_HEIGHT) /  `MAILBOX_STREAM_SIZE)
`define MAILBOX_COUNTER_WIDTH ($clog2(`MAILBOX_STREAM_CYCLES + 1))
