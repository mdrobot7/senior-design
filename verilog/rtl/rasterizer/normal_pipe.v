module normal_pipe_m(
    ...
    reg sub;
    reg  signed [2 * `WORD_WIDTH - 1:0] as1a; reg  signed [2 * `WORD_WIDTH - 1:0] as1b;
    wire signed [2 * `WORD_WIDTH - 1:0] as1y;
    addsub_m #(2 * `WORD_WIDTH) addsub1( .sub_i(sub), .a_i(as1a), .b_i(as1b), .y_o(as1y) );

    reg m1vp;
    reg  signed [`WORD_WIDTH - 1:0] m1a; reg  signed [`WORD_WIDTH - 1:0] m1b;
    wire signed [2 * `WORD_WIDTH - 1:0] m1y;
    mul_m #(`WORD_WIDTH, `WORD_WIDTH * 2) mul1(
        .a_i(m1a), .b_i(m1b), .y_o(m1y)
    );

    reg  [`WORD] sqrta;
    reg          sqrt_in_valid, sqrt_out_ready;
    wire         sqrt_out_valid, sqrt_in_ready;
    wire [`WORD_WIDTH - `DECIMAL_POS / 2 - 1:0] sqrty;
    ...
    assign sqrty = sqrt_mo[`STREAM_MO_DATA(`WORD_WIDTH / 2)] << (`DECIMAL_POS / 2);
    ...
    reg signed [`WORD] vax, vay, vaz;
    reg signed [`WORD] vbx, vby, vbz;

    reg signed [2 * `WORD_WIDTH - 1:0] nx, ny, nz;
