module tex_sample_m(
    ...
    reg [`WORD_WIDTH - 1:0] u0, u1, u2, u3, u4, u5;
    ...
    wire [(`WORD_WIDTH / 2) - 1:0] posx_ext, posy_ext;
    assign posx_ext = posx;
    assign posy_ext = posy;

    wire [`FRAGMENT_WIDTH - 1:0] out_data;
    assign out_data = {
        u5, u4, u3, u2, u1, u0,
        posy_ext, posx_ext, // half width
        color_ext
    };

    assign mstream_o[`STREAM_MO_VALID(`FRAGMENT_WIDTH)] = out_ready;
    assign mstream_o[`STREAM_MO_DATA(`FRAGMENT_WIDTH)]  = out_data;
            ...
            case (state)
                0: begin
                    if (sstream_i[`STREAM_SI_VALID(`RAST_WAVG_OUT_WIDTH)]) begin
                        ...
                        { posx, posy, tx, ty, u0, u1, u2, u3, u4, u5 } = sstream_i[`STREAM_SI_DATA(`RAST_WAVG_OUT_WIDTH)];
    ...
    assign sstream_o[`STREAM_SO_READY(`RAST_WAVG_OUT_WIDTH)] = state == 0;
