...
`endif
    ...
    `ifdef HARDENING
        ...
        `endif
            ...
            // Skip power pin connections for rtl/fpga
            .clk_i(clk_i),
            .addr_i(imem_addr),
            .read_en_i(imem_rw),
            .en_i(1'b1),
    ...
    // Global regfile
    wire                         global_regfile_write_en    = (!enable_i && state == STATE_READY) ? global_regfile_write_en_i : 0;
    wire [`REG_SOURCE_WIDTH-1:0] global_regfile_r1_addr     = (!enable_i && state == STATE_READY) ? global_regfile_addr_i     : inst_rs1;
    wire [`WORD]                 global_regfile_r1_data;
    assign                       global_regfile_read_data_o = (!enable_i && state == STATE_READY) ? global_regfile_r1_data    : 0;
