module vertex_deserializer_m(
    ...
    reg [$clog2(`OUTBOX_STREAM_CYCLES + 1) - 1:0] pos;
    ...
    always @(posedge clk_i, negedge nrst_i) begin
        if (!nrst_i) begin
            ...
            if (pos < `OUTBOX_STREAM_CYCLES) begin
                if (sstream_i[`STREAM_SI_VALID(`MAILBOX_STREAM_SIZE)]) begin
                    pos <= pos + 1;

                    out_data[pos * `MAILBOX_STREAM_SIZE+:`MAILBOX_STREAM_SIZE]
                        <= sstream_i[`STREAM_SI_DATA(`MAILBOX_STREAM_SIZE)];
                end
            end
            else begin
                if (mstream_i[`STREAM_MI_READY(`SHADED_VERTEX_WIDTH)]) begin
                    pos <= 0;
                end
            end
        end
    ...
    assign sstream_o[`STREAM_SO_READY(`MAILBOX_STREAM_SIZE)] = pos < `OUTBOX_STREAM_CYCLES;

    assign mstream_o[`STREAM_MO_DATA(`SHADED_VERTEX_WIDTH)] = out_data;
    assign mstream_o[`STREAM_MO_VALID(`SHADED_VERTEX_WIDTH)] = pos == `OUTBOX_STREAM_CYCLES;
