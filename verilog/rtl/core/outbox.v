module outbox_m (
    ...
    input  wire [`CORE_OUTBOX_HEIGHT*`WORD_WIDTH-1:0] outbox_i
);

    reg ready; // 1: Ready to accept new data from the core.

    reg [`MAILBOX_STREAM_SIZE - 1:0] outbox [`OUTBOX_STREAM_CYCLES - 1:0];

    reg [`OUTBOX_COUNTER_WIDTH:0] counter;


    always @(*) begin : ASSIGNMENT
        ready <= (counter == `OUTBOX_STREAM_CYCLES);
    ...
    always @(posedge clk_i, negedge nrst_i) begin
        if(!nrst_i) begin : RESET
            integer i;
            for( i = 0; i < `OUTBOX_STREAM_CYCLES; i = i + 1)
                outbox[i] = 0;

            counter <= `OUTBOX_STREAM_CYCLES;
        end
            ...
            if (!nsync_rst_i) begin : SYNC_RESET
                ...
                for(i = 0; i < `OUTBOX_STREAM_CYCLES; i = i + 1) begin
