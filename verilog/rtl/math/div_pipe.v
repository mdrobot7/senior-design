...
`ifdef SIM_DIV
    ...
    wire [WIDTH - 1:0] au, bu;
    assign au = in_data[1 * WIDTH+:WIDTH];
    assign bu = in_data[0 * WIDTH+:WIDTH];
    
    wire signed [WIDTH - 1:0] y;

    assign y = (sstream_i[`STREAM_SI_LAST(IN_SIZE)]) ? (au / bu) : (a / b);
...
`else
    ...
    always @(*) begin
        ...
        if (!sstream_i[`STREAM_SI_LAST(IN_SIZE)]) begin
            negate[0]       <= (a < 0) ^ (b < 0);
    ...
    always @(*) begin
        ...
        if (divisor[WIDTH] == 0) begin
            mstream_o[`STREAM_MO_DATA(OUT_SIZE)] <= { extra[WIDTH], { WIDTH { 1'b0 } } };
        end
        else begin : YEP
            reg signed [WIDTH - 1:0] gaming;

            if (negate[WIDTH]) begin
                gaming = -quotient[WIDTH];
            end
            else begin
                gaming = quotient[WIDTH];
            end

            mstream_o[`STREAM_MO_DATA(OUT_SIZE)]  <= { extra[WIDTH], gaming };
        end
