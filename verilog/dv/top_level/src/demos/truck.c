...
static void truck_setup() {
    ugpu_wb_pk_memcpy(TEX_BUFFER_PK_ADDR, (uint32_t *) DuweTruck_t, DUWETRUCK_SIZE_X * DUWETRUCK_SIZE_Y / 4);

    RAST->TEXADDR.reg   = TEX_BUFFER_PK_ADDR;
    RAST->TEXWIDTH.reg  = DUWETRUCK_SIZE_X;
    RAST->TEXHEIGHT.reg = DUWETRUCK_SIZE_Y;
    RAST->DEPTHADDR.reg = DEPTH_PK_ADDR;
    RAST->USERCOUNT.reg = 3;

    ugpu_wb_pk_memcpy(VERTEX_BUFFER_PK_ADDR, (uint32_t *) DuweTruck_vertices, (DUWETRUCK_VERTICES * sizeof(vertex_t)) / 4);
    ugpu_wb_pk_memcpy(INDEX_BUFFER_PK_ADDR, (uint32_t *) DuweTruck_triangles, (DUWETRUCK_TRIANGLES * sizeof(triangle_t)) / 4);
}

static void truck_loop() {
    mat4_t mvp;

    mat4_t rot;

    mat4_t trans;
    mat4_trans(trans, 0, UGPU_FIXED(5), UGPU_FIXED(-10));
    ...
    mat4_rotated(rot, 2 * UGPU_FIXED(x % 180), 4 * UGPU_FIXED(x % 90), UGPU_FIXED(x));
    ...
    while (1) {
        ...
        mat4_rotated(rot, UGPU_FIXED(180), UGPU_FIXED(x), UGPU_FIXED(0));
        mat4_muld(mvp, post_rot, rot);
