...
static void cow_setup() {
    ugpu_wb_pk_memcpy(TEX_BUFFER_PK_ADDR, (uint32_t *) cow_t, COW_SIZE_X * COW_SIZE_Y / 4);

    RAST->TEXADDR.reg   = TEX_BUFFER_PK_ADDR;
    RAST->TEXWIDTH.reg  = COW_SIZE_X;
    RAST->TEXHEIGHT.reg = COW_SIZE_Y;
    RAST->DEPTHADDR.reg = DEPTH_PK_ADDR;
    RAST->USERCOUNT.reg = 3;

    ugpu_wb_pk_memcpy(VERTEX_BUFFER_PK_ADDR, (uint32_t *) cow_vertices, (COW_VERTICES * sizeof(vertex_t)) / 4);
    ugpu_wb_pk_memcpy(INDEX_BUFFER_PK_ADDR, (uint32_t *) cow_triangles, (COW_TRIANGLES * sizeof(triangle_t)) / 4);
}

static void cow_loop() {
    mat4_t mvp;

    mat4_t rot;

    mat4_t trans;
    mat4_trans(trans, 0, 0, UGPU_FIXED(-6));
    ...
    int swap = 0;
    mat4_rotated(rot, UGPU_FIXED(x), UGPU_FIXED(y), UGPU_FIXED(z));
