...
static void earth_setup() {
    ugpu_wb_pk_memcpy(TEX_BUFFER_PK_ADDR, (uint32_t *) earth_t, EARTH_SIZE_X * EARTH_SIZE_Y / 4);

    RAST->TEXADDR.reg   = TEX_BUFFER_PK_ADDR;
    RAST->TEXWIDTH.reg  = EARTH_SIZE_X;
    RAST->TEXHEIGHT.reg = EARTH_SIZE_Y;
    RAST->DEPTHADDR.reg = DEPTH_PK_ADDR;
    RAST->USERCOUNT.reg = 3;

    ugpu_wb_pk_memcpy(VERTEX_BUFFER_PK_ADDR, (uint32_t *) earth_vertices, (EARTH_VERTICES * sizeof(vertex_t)) / 4);
    ugpu_wb_pk_memcpy(INDEX_BUFFER_PK_ADDR, (uint32_t *) earth_triangles, (EARTH_TRIANGLES * sizeof(triangle_t)) / 4);
}

static void earth_loop() {
    mat4_t mvp;
