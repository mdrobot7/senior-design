...
static void suzanne_setup() {
    ugpu_wb_pk_memcpy(TEX_BUFFER_PK_ADDR, (uint32_t *) suzanne_t, SUZANNE_SIZE_X * SUZANNE_SIZE_Y / 4);

    RAST->TEXADDR.reg   = TEX_BUFFER_PK_ADDR;
    RAST->TEXWIDTH.reg  = SUZANNE_SIZE_X;
    RAST->TEXHEIGHT.reg = SUZANNE_SIZE_Y;
    RAST->DEPTHADDR.reg = DEPTH_PK_ADDR;
    RAST->USERCOUNT.reg = 3;

    ugpu_wb_pk_memcpy(VERTEX_BUFFER_PK_ADDR, (uint32_t *) suzanne_vertices, (SUZANNE_VERTICES * sizeof(vertex_t)) / 4);
    ugpu_wb_pk_memcpy(INDEX_BUFFER_PK_ADDR, (uint32_t *) suzanne_triangles, (SUZANNE_TRIANGLES * sizeof(triangle_t)) / 4);
}

static void suzanne_loop() {
    mat4_t mvp;

    mat4_t rot;

    mat4_t trans;
    mat4_trans(trans, 0, 0, UGPU_FIXED(-2));
