...
static void cheburashka_setup() {
    ugpu_wb_pk_memcpy(TEX_BUFFER_PK_ADDR, (uint32_t *) cheburashka_t, CHEBURASHKA_SIZE_X * CHEBURASHKA_SIZE_Y / 4);

    RAST->TEXADDR.reg   = TEX_BUFFER_PK_ADDR;
    RAST->TEXWIDTH.reg  = CHEBURASHKA_SIZE_X;
    RAST->TEXHEIGHT.reg = CHEBURASHKA_SIZE_Y;
    RAST->DEPTHADDR.reg = DEPTH_PK_ADDR;
    RAST->USERCOUNT.reg = 3;

    ugpu_wb_pk_memcpy(VERTEX_BUFFER_PK_ADDR, (uint32_t *) cheburashka_vertices, (CHEBURASHKA_VERTICES * sizeof(vertex_t)) / 4);
    ugpu_wb_pk_memcpy(INDEX_BUFFER_PK_ADDR, (uint32_t *) cheburashka_triangles, (CHEBURASHKA_TRIANGLES * sizeof(triangle_t)) / 4);
}

static void cheburashka_loop() {
    mat4_t mvp;

    mat4_t rot;

    mat4_t trans;
    mat4_trans(trans, 0, UGPU_FLOAT_TO_FIXED(-0.5), UGPU_FIXED(-1));
