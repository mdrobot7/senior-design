...
static void duwe_setup() {
    ugpu_wb_pk_memcpy(TEX_BUFFER_PK_ADDR, (uint32_t *) Duwe_t, DUWE_SIZE_X * DUWE_SIZE_Y / 4);

    RAST->TEXADDR.reg   = TEX_BUFFER_PK_ADDR;
    RAST->TEXWIDTH.reg  = DUWE_SIZE_X;
    RAST->TEXHEIGHT.reg = DUWE_SIZE_Y;
    RAST->DEPTHADDR.reg = DEPTH_PK_ADDR;
    RAST->USERCOUNT.reg = 3;

    ugpu_wb_pk_memcpy(VERTEX_BUFFER_PK_ADDR, (uint32_t *) Duwe_vertices, (DUWE_VERTICES * sizeof(vertex_t)) / 4);
    ugpu_wb_pk_memcpy(INDEX_BUFFER_PK_ADDR, (uint32_t *) Duwe_triangles, (DUWE_TRIANGLES * sizeof(triangle_t)) / 4);
}

static void duwe_loop() {
    mat4_t mvp;

    mat4_t rot;

    mat4_t trans;
    mat4_trans(trans, 0, UGPU_FIXED(0), UGPU_FIXED(-2));
