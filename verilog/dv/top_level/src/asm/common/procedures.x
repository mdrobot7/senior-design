...
#define DOUBLE_TO_FIXED(n) ((int32_t) ((double) (n) * (1 << DECIMAL_POS)))
#define FIXED_TO_DOUBLE(n) ((double) (n) / (1 << DECIMAL_POS))
#define EPSILON            (0.005f)
#define DOUBLE_EQUAL(a, b) (fabs((a) - (b)) < EPSILON)
const int32_t fixedtests[] = { DOUBLE_TO_FIXED(0), DOUBLE_TO_FIXED(1), DOUBLE_TO_FIXED(2), DOUBLE_TO_FIXED(9), DOUBLE_TO_FIXED(10), DOUBLE_TO_FIXED(1000), DOUBLE_TO_FIXED(10000), DOUBLE_TO_FIXED(32767), DOUBLE_TO_FIXED(-32678), DOUBLE_TO_FIXED(-16555.123), DOUBLE_TO_FIXED(-12345), DOUBLE_TO_FIXED(-2), DOUBLE_TO_FIXED(-1) };
...
int test_fixeddiv() {
  for (int i = 0; i < NUM_TESTS; i++) {
    for (int j = 0; j < NUM_TESTS; j++) {
      ...
      if (i32 == DOUBLE_TO_FIXED(-2097152) && j32 == DOUBLE_TO_FIXED(-1)) {
        // Special case: INT_MIN / -1 = INT_MIN, doubles don't understand this.
        if (quotient != DOUBLE_TO_FIXED(-2097152)) {
          printf("FAIL: %f / %f => %f, expected %f\n", FIXED_TO_DOUBLE(i32), FIXED_TO_DOUBLE(j32), FIXED_TO_DOUBLE(quotient), -2097152.0f);
          return 1;
        }
        continue;
      }

      double expected = FIXED_TO_DOUBLE(i32) / FIXED_TO_DOUBLE(j32);

      if (!DOUBLE_EQUAL(FIXED_TO_DOUBLE(quotient), expected)) {
        printf("FAIL: %f / %f => %f, expected %f\n", FIXED_TO_DOUBLE(i32), FIXED_TO_DOUBLE(j32), FIXED_TO_DOUBLE(quotient), expected);
        printf("FAIL: 0x%x / 0x%x => 0x%x, expected 0x%x\n", i32, j32, quotient, (i32 / j32));
        return 1;
      }
    }
...
int test_fixedmod() {
  for (int i = 0; i < NUM_TESTS; i++) {
    for (int j = 0; j < NUM_TESTS; j++) {
      ...
      double expected = fmod(FIXED_TO_DOUBLE(i32), FIXED_TO_DOUBLE(j32));
