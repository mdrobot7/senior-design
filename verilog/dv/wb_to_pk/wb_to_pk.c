...
void main() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (ADDR != current_addr) test_fail();
    if (RDATA != 0xFAFAFAFA) test_fail();
    if (readValue != 0xFAFAFAFA) test_fail();
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (ADDR != current_addr) test_fail();
    if (RDATA != 0x45520080) test_fail();
    if (readValue != 0x45520080) test_fail();
