module rasterizer_tb();
    ...
    rasterizer_m rasterizer(
        ...
        .depth_addr_i(`ADDR_DEPTH_BUFFER),
    ...
    stream_stat_m #(`RAST_INTERP_OUT_WIDTH) wavg_stat(
        .clk_i(clk),

        .mstreami_i(rasterizer.interp_streami),
        .mstreamo_i(rasterizer.interp_streamo)
    );

    stream_stat_m #(`RAST_INTERP_OUT_WIDTH) interp_fifo_stat(
        .clk_i(clk),

        .mstreami_i(rasterizer.interp_fifo_streami),
        .mstreamo_i(rasterizer.interp_fifo_streamo)
    );
    ...
    initial begin : MAIN
        ...
        for (x = 0; x < 320; x = x + 1) begin
            for (y = 0; y < 240; y = y + 1) begin
                WRITE_MEM(`ADDR_FB0 + (y * 320 + x), 8'b11000000);
            end
        end
